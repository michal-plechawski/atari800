...
//  LEVELS MANAGEMENT:
    ...
    static const float levels_autodim_temp = 1.0;               //  range (0, 1] default is 0.5 but that was unnecessarily dark for me, so I set it to 1.0
...
/***************** BEGIN phosphor-mask-resizing.h ******************/
...
void main()
{
   ...
   float2 tex_uv = TEX0.xy;
    ...
    const float2 tile_uv_wrap_local = output_video_uv * output_tiles_this_pass;
    ...
    src_tex_uv_wrap = tile_uv_wrap_local * tile_size_uv;

    //  Output the values we need, including the magnification scale and step:
    tile_uv_wrap = tile_uv_wrap_local;
